...
{
int
composeGTML (const std::string &filepath, Dom *document)
{
  ...
  file.write ("gtml", 4);
  file.write (reinterpret_cast<char *> (&major_version),
              sizeof (major_version));
  file.write (reinterpret_cast<char *> (&minor_version),
              sizeof (minor_version));
  file.write (reinterpret_cast<char *> (&patch_version),
      ...
      if (elements[i]->parent () != nullptr)
        {
          ...
          file.write (reinterpret_cast<char *> (&parent_index),
                      sizeof (parent_index));
        }
      else
        {
